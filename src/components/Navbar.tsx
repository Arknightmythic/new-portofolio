        ...
        <header className="fixed top-0 left-0 w-full z-50 bg-[#F5F7FA] text-[#0A192F]">
            ...
            <div className="flex items-center justify-between px-4 py-4 max-w-6xl mx-auto">
                ...
                <button onClick={() => setIsOpen(!isOpen)} className="inline-block">
                    {isOpen ? (<span className="flex flex-row gap-3"><X className="w-6 h-6 border-2 rounded-[5px]" /> <h4>Close</h4></span>) : (<span className="flex flex-row gap-3"><Menu className="w-6 h-6 border-2 rounded-[5px]" /> <h4>Menu</h4></span>)}
                </button>
                    ...
                    <ul className="space-y-2 text-[#F5F7FA]">
                        {navItems.map((item) => (
                            <li key={item.href}>
                                <a
                                    href={item.href}
                                    className="block text-sm py-2 border-b border-white/20"
